...
import com.microsoft.azure.schemaregistry.spark.avro.AvroUtils.{toFieldStr, AvroMatchedField}
...
class AvroSerializer(
    ...
    (catalystType, avroType.getType) match {
      ...
      case (TimestampType, LONG) => avroType.getLogicalType match {
        ...
        case null | _: TimestampMillis => (getter, ordinal) =>
          Math.floorDiv(getter.getLong(ordinal), 1000L)
  ...
  private def newStructConverter(
    ...
    val avroSchemaHelper = new AvroUtils.AvroSchemaHelper(
      avroStruct, catalystStruct, avroPath, catalystPath)
